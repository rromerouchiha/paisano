...
$sql['estados_m'] = 'select e.id,e.nombre_estado from pais p inner join estado e on p.id = e.pais_id where p.clave = "MEX" order by nombre_estado;';
...
$sql['estados_solo_df'] = 'select e.id,e.nombre_estado from pais p inner join estado e on p.id = e.pais_id where e.id = 32 order by nombre_estado;';
